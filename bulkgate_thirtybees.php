...
class BulkGate_ThirtyBees extends Module
{
    ...
    {
        $this->name = _BULKGATE_SLUG_;
        $this->version = _BULKGATE_VERSION_;
        $this->author = _BULKGATE_AUTHOR_;
        $this->tab = 'emailing';
        $this->author_uri = _BULKGATE_AUTHOR_URL_;
        $this->ps_versions_compliancy = [
            'min' => _BULKGATE_PS_MIN_VERSION_,
            'max' => _PS_VERSION_,
        ];

        parent::__construct();

        $this->ps_di = new ThirtyBeesSms\DIContainer(\Db::getInstance());
    ...
    public function getContent()
    {
        ...
        {
            \Tools::redirectAdmin($this->context->link->getAdminLink('AdminBulkGateModuleSettingsDefault'));
        }
        \Tools::redirectAdmin($this->context->link->getAdminLink('AdminBulkGateSignIn'));
    }
    ...
    public function hookActionBulkGateSendSms(array $params)
    {
    ...
    public function hookDisplayAdminOrderRight(array $params)
    {
        if($this->ps_settings->load("static:application_token", false))
        {
            if(isset($params['id_order']))
            {
                list($phone_number, $iso) = ThirtyBeesSms\Helpers::getOrderPhoneNumber($params['id_order']);
            }
            else
            {
            ...
            $controller = 'AdminBulkGateDashboardDefault';

            $this->context->smarty->registerPlugin('modifier', 'bulkGateEscapeHtml', array('BulkGate\Extensions\Escape', 'html'));
            $this->context->smarty->registerPlugin('modifier', 'bulkGateEscapeJs', array('BulkGate\Extensions\Escape', 'js'));
            $this->context->smarty->registerPlugin('modifier', 'bulkGateEscapeUrl', array('BulkGate\Extensions\Escape', 'url'));
            $this->context->smarty->registerPlugin('modifier', 'bulkGateEscapeHtmlAttr', array('BulkGate\Extensions\Escape', 'htmlAttr'));
            $this->context->smarty->registerPlugin('modifier', 'bulkGateTranslate', array($this->ps_di->getTranslator(), 'translate'));

            return $this->context->smarty->createTemplate(_BULKGATE_DIR_.'/templates/panel.tpl', null, null, array(
                ...
                'proxy' => array(),
                'salt' => Extensions\Compress::compress(ThirtyBeesSms\Helpers::generateTokens()),
                'authenticate' => array(
                    'ajax' => true,
                ...
                'homepage' => $this->context->link->getAdminLink('AdminBulkGateDashboardDefault'),
    ...
    public function hookActionBulkGateExtendsVariables(array $params)
    {
    }
    ...
    public function runHook($name, Extensions\Hook\Variables $variables)
    {
        $hook = new Extensions\Hook\Hook(
            ...
            $this->ps_settings,
            new ThirtyBeesSms\HookLoad($this->ps_di->getDatabase(), $this->context)
        );
