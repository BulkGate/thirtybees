 ...
 */
class ThirtyBeesSMS extends Strict implements IModule
{
    const PRODUCT = 'tb';

    private $info = array(
        'store' => 'ThirtyBees',
        'name' => _BULKGATE_NAME_,
    ...
    public function info($key = null)
    {
        if(empty($this->plugin_data))
        {
            $this->plugin_data = array_merge(
                array(
                    'store_version' => defined('_BULKGATE_PS_MIN_VERSION_') ? (_BULKGATE_PS_MIN_VERSION_.'+') : '1.7',
                    'version' => _BULKGATE_VERSION_,
